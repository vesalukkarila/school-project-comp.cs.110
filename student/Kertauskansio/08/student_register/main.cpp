...
int main() {
    ...
    std::map< std::string, Student* > user_ids; //avaimena string (userid tai studentnumber) ja hkuormana osoitin dyn.tietueeseen
    ...
    while(true) {
        ...
        } else if(command == "C" or command == "c")
        {
            ...
            //etsitään löytyykö puhelinnumerolla opiskelijaa
            bool loytyy = false;
            ...
            if (student_numbers.find(parts.at(1)) != student_numbers.end())
            {
                ...
                getline(cin, uusinumero);
                if (!is_valid_phone_number(uusinumero))
                {
                    cout << "Erroneous phone number: "<< uusinumero << endl;
                    continue;
                }
                student_numbers.at(parts.at(1))->phone_number = uusinumero;

                ofstream tiedosto_olio(file_name);
                for (auto tietopari : user_ids)
                {
                    tiedosto_olio << tietopari.second->student_number << ";" << tietopari.second->user_id <<";"<<
                                     tietopari.second->name << ";"<< tietopari.second->phone_number<< ";"<<
                                     tietopari.second->email << ";" << tietopari.second->skype << endl;
                }
                tiedosto_olio.close();
            }
            else
